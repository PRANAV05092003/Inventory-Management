import { extractItems, normalizePaginatedResponse } from '../utils/apiResponse';
...
const EMPTY_STOCK_DISTRIBUTION = { low: 0, medium: 0, healthy: 0, total: 0 };

function emptyDashboard() {
  const kpis = buildKpiMeta({
    totalProducts: 0,
    totalCustomers: 0,
    totalOrders: 0,
    totalUnits: 0,
  ...
  return {
    ...
    averageOrderValue: 0,
    kpis,
    stockDistribution: { ...EMPTY_STOCK_DISTRIBUTION },
    criticalStock: [],
    recentOrders: [],
    activityFeed: [],
    topProducts: [],
...
async function fetchAllPages(fetcher) {
  ...
  do {
    const raw = await fetcher({ page, size: PAGE_SIZE });
    const { items: pageItems, total: pageTotal } = normalizePaginatedResponse(raw);
...
function computeStockDistribution(products) {
  if (!Array.isArray(products) || products.length === 0) {
    return { ...EMPTY_STOCK_DISTRIBUTION };
  }
  ...
  let healthy = 0;
  products.forEach((p) => {
    const q = Number(p?.quantity_in_stock ?? 0);
...
function buildActivityFeed(products, customers, orders, lowStockProducts) {
  ...
  const safeLow = Array.isArray(lowStockProducts) ? lowStockProducts : [];
...
export async function fetchDashboardData() {
  try {
    ...
    const products = extractItems(productsResult);
    const customers = extractItems(customersResult);
    const orders = extractItems(ordersResult);

    const totalProducts = Number(productsMeta?.total) || 0;
    const totalCustomers = Number(customersMeta?.total) || 0;
    const totalOrders = Number(ordersMeta?.total) || 0;
    const totalUnits = products.reduce((sum, p) => sum + Number(p?.quantity_in_stock ?? 0), 0);

    const productMap = Object.fromEntries(
      products.filter((p) => p?.id).map((p) => [p.id, p.name]),
    );
    const customerMap = Object.fromEntries(
      customers.filter((c) => c?.id).map((c) => [c.id, c.full_name]),
    );

    const inventoryValue = products.reduce(
      (sum, p) => sum + Number(p?.price ?? 0) * Number(p?.quantity_in_stock ?? 0),
      0,
    );

    const lowStockProducts = products
      .filter((p) => Number(p?.quantity_in_stock ?? 0) < LOW_STOCK_THRESHOLD)
    ...
    const averageOrderValue =
      totalOrders > 0
        ? orders.reduce((s, o) => s + Number(o?.total_amount ?? 0), 0) / totalOrders
        : 0;
...
/** @alias fetchDashboardData */
export const getDashboardMetrics = fetchDashboardData;
